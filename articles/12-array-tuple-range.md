...
仓颉使用 `Array<T>` 表示数组类型，`T` 是元素类型，可以是任意类型。Array 是引用类型，但其内部持有的是元素的引用（不是 struct 的逐元素拷贝）。
...
### Q8: Range 是 Iterable 吗？

是的。Range 实现了迭代器协议，可用 `for-in` 遍历，也支持 `Iterator` 接口的 `next()` 等方法（详见标准库）。
