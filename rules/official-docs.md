...
每次新增文章主题、调整文章边界或官方版本更新时，必须重新打开官方目录进行一次全量查漏：不能只核对当前文章对应的单个章节，必须确认没有新增或遗漏的官方章节。

每完成一个小阶段，必须执行小阶段复盘门禁：对照官方目录复核当前阶段是否遗漏或误归类、对照覆盖矩阵检查已完成内容的事实与链接、复核下一阶段是否需要补充标准库、工具或实践主题，并在更新计划与状态后与用户确认再进入下一阶段。
