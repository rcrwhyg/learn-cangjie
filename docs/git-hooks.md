...
pre-push 只证明本地测试通过，不代表远程流程已经完成。推送成功后，必须继续检查对应的 GitHub Actions；只有远程工作流全部成功，提交才算验证完成。
...
如果 Actions 失败，必须查看失败 job 日志，修复根因，创建新的修复提交并重新推送；不能只依赖本地钩子通过，也不能用 `--no-verify` 绕过验证。
